...
新增业务域、安全事件或重要操作时，必须同步更新以下两处：

1. **`app/plugins/controller.plug.ts`** — 在 `AUDIT_REGISTRY` 数组中追加条目：
   - `prefix`：URL 路径前缀
   - `category`：对应审计类别
   - `methods`：HTTP 方法 → 审计动作映射（固定字符串或根据路径推导的函数）

2. **`app/lib/audit.ts`** — 如新增类别需扩展 `AuditCategory` 联合类型。

审计日志文件位于 `logs/audit/{category}/`，单文件上限 10MB，超限自动轮转。
文件名格式：`YYYY-MM-DDTHH-mm-ss-SSSZ.log`，按年筛选用前缀匹配。

```typescript
// 示例：新增 "comment" 业务域
// 1. controller.plug.ts AUDIT_REGISTRY 追加
{
  prefix: "/v1/comments",
  category: "prompt",  // 或新建 "comment" 类别（需同步改 AuditCategory）
  methods: { POST: "create", PUT: "update", DELETE: "delete" },
}
// 2. audit.ts 如新增类别
export type AuditCategory = "auth" | "novel" | "prompt" | "comment" | "security" | "system";
```
